...
from commodity_price_simulation import simulate_commodity_prices, get_price_statistics
from stochastic_decline_curve import get_decline_curve_statistics
...
def main():
    ...
    print_subheader("Simulating: Commodity prices + Decline curve steepening\n")
    ...
    print(f"  Avg Loss (if loss):   ${avg_loss:.1f}MM")
    ...
    print("-" * 80)
    print(f"{'Pctl':<6} {'IRR':>7} | {'Price Range':^16} | {'b-factor Range':^16} | {'n':>5}")
    print("-" * 80)
    ...
    for p in percentiles:
        ...
        print(f"{label:<6} {data['irr']:>6.1%} | {price_range:^16} | {b_range:^16} | {data['n_sims']:>5}")
    
    print("-" * 80)
    print(f"Base Case: Price=1.00x, b=0.90 -> IRR={base_irr:.1%}")
    ...
    # 3B: Decline Curve Scenarios
    print("\n3B. DECLINE CURVE STEEPENING SCENARIOS (Strip Pricing)")
    ...
    # 3C: Combined Stress Scenarios
    print("\n3C. COMBINED STRESS SCENARIOS (Price + Decline)")
    ...
    # ==========================================================================
    # SECTION 5: GENERATE CHARTS
    # ==========================================================================
    
    print_header("SECTION 5: GENERATING CHARTS...")
